...
图标隐藏后拦截**依然生效**,但菜单栏上没有入口了。为了不会「藏丢」,做了一个逃生窗口:

- **每次启动应用,图标都会先显示 10 秒**,菜单顶部同时提示「⏱ 菜单栏图标将在启动 10 秒后隐藏」
- 这 10 秒内**打开一次菜单**即取消本次隐藏,图标保留到退出为止
- 想找回图标:**退出并重新打开 慢Q**(或重新登录)即可

> 实现细节:macOS 会持久化状态栏项的可见性,隐藏过一次后新建的实例默认仍是隐藏的,因此代码里每次启动都显式置为可见,再由定时器决定是否隐藏。
